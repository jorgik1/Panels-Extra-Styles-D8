...
class WrapperElementStyle extends PanelsStyleBase implements ContainerFactoryPluginInterface {
   ...
   *   Renderer.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, CurrentRouteMatch $routeMatch, Renderer $renderer) {
  ...
  public function buildBlock(PanelsDisplayVariant $display, BlockPluginInterface $block) {
    ...
    if (is_array($config) && !empty($config['style']['configuration']['pane']['content']['element'])) {
      $build['content'] = [
        '#markup' => $this->renderer->render($render_array),
        '#prefix' => sprintf('<%s id="%s" class="%s">', $config['style']['configuration']['pane']['content']['element'] ?: 'div',
          Html::getId($config['style']['configuration']['pane']['content']['attributes']['id']),
          Html::cleanCssIdentifier($config['style']['configuration']['pane']['content']['attributes']['class'])
        ),
        '#suffix' => "</{$config['style']['configuration']['pane']['content']['element']}>" ?: '</div>',
      ];
    }
    elseif ($config['style']['configuration']['pane']['content']['element'] == 0) {
      $build['content'] = [
        '#markup' => $this->renderer->render($render_array),
      ];
    }
      ...
      $build['content'] += [
        '#title' => [
          ...
          '#prefix' => sprintf('<%s id="%s" class="%s">', !empty($config['style']['configuration']['pane']['title']['element']) ?: 'h2',
            Html::getId($config['style']['configuration']['pane']['title']['attributes']['id']),
            Html::cleanCssIdentifier($config['style']['configuration']['pane']['title']['attributes']['class'])
          ),
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static($configuration, $plugin_id, $plugin_definition, $container->get('current_route_match'), $container->get('renderer'));
  }
